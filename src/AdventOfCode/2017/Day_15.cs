...
public class Day_15
{
    ...
    public int part_one(string input) => Match(input, 40_000_000, n => true, n => true);
    ...
    public int part_two(string input) => Match(input, 5_000_000, n => (n & 3) == 0, n => (n & 7) == 0);

    private static int Match(string input, int pairs, Predicate<long> ca, Predicate<long> cb)
    {
        var numbers = input.Lines().Int32s().ToArray();
