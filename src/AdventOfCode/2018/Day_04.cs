...
public class Day_04
{
    ...
    public int part_one(Inputs<Log> logs) => Occurences(logs).OrderByDescending(c => c.Value.Total).Select(Score).First();
    ...
    public record Log(DateTime Timestamp, string Text)
    {
    ...
    public enum Action { Begins, Sleeps, WakesUp }
}
