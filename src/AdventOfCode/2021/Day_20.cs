...
public class Day_20
{
    ...
    {
        var lookup_odd = lines[0][0].Select(c => c == '#').ToArray();
        var lookup_evn = lookup_odd.ToArray();

        if (lookup_odd[0]) // all off pixels are on during odd turns
        {
            lookup_odd = [.. lookup_odd.Select(toggle => !toggle)];
            lookup_evn = [.. lookup_odd.Select((toggle, index) => new { toggle, index })
        ...
        foreach (var point in map.Keys.SelectMany(p => Neighbors.Select(n => p + (n * 2)).Where(n => !map.ContainsKey(n))).Fix())
        {
            map[point] = false;
        }
        for (var turn = 1; turn <= turns; turn++)
        {
            ...
            var lookup = turn.IsOdd ? lookup_odd : lookup_evn;
