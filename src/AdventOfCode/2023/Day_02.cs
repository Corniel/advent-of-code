...
public class Day_02
{
    ...
    public int part_one(Lines input)
        => input.Select(Game.parse).Where(g => g.Hands.TrueForAll(h => h.Fits(fit))).Sum(g => g.Id);
    
    [Example(answer: 2286, Example._1)]
    [Puzzle(answer: 62811, O.μs100)]
    public int part_two(Lines input) => input.Select(Game.parse).Select(g => g.Smallest.Pow).Sum();
