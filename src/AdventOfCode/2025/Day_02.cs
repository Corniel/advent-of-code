...
public class Day_02
{
    ...
    public long part_one(Longs numbers) => Ids(numbers).Where(One).Sum();
    ...
    public long part_two(Longs numbers) => Ids(numbers).Where(Two).Sum();
    ...
    // Due to the dash in the input, the second number is negated by the parser.
    static IEnumerable<long> Ids(Longs ns)
    {
        for (var i = 0; i < ns.Count - 1; i += 2)
            for (var n = ns[i]; n <= -ns[i + 1]; n++)
                yield return n;
    }
}
